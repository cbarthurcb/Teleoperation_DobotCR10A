"""
CABEÇALHO DOCUMENTAÇÃO - Controle Dobot CR10A & Célula de Carga ESP32
SISTEMA: Compressão por Tração (Subir aumenta carga / Descer alivia carga)
VERSÃO: v6 — Stop() na mudança de direção do comando PI
CORREÇÕES:
  - Stop() ao inverter direção: limpa fila do Dobot a cada inversão do PI
  - Travamento de fase (latch): após detectar contato, nunca volta para aproximação
  - Stop() na transição de aproximação para controle
  - Reset de integral na mudança de sinal do erro
  - Anti-windup condicional (não acumula quando saída está saturada)
  - Fase de aproximação com passo fixo (sem windup antes do contato)
  - dt real medido por ciclo
  - Parâmetros atualizados em tempo real sem reiniciar o loop
  - Botões +/- para ajuste fino de todos os parâmetros
"""
...
class PIController:
    def __init__(self, kp, ki, dt, max_output=2.0):
        ...
        self.setpoint = 0.0
        self.integral = 0.0
        self.deadzone = 0.02  # 20 gramas de tolerância
...
class DobotAPI:
    ...
    def stop(self):
        ...
        try:
            ...
            time.sleep(0.1)  # aguarda o robô processar o stop
        except:
            pass
...
class DobotControlApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Controle de Força V6 - Dobot CR10A")
    ...
    # --------------------------------------------------------------------------
    # Utilitário: linha de parâmetro com botões − / +
    # --------------------------------------------------------------------------
